...
class Voxelizer:
    ...
    def __init__(self, pdb_structure, config, use_cache=None, write_cache=True):
        ...
        if use_cache_voxels and os.path.isfile(check_oeb):
            ...
            prot_vox, prot_centers, prot_N = getVoxelDescriptors(prot, buffer=0, voxelsize=config['voxelsize'],
    ...
    def __call__(self, lig_pdb, quantity='all'):
        ...
        lig_vox, lig_centers, lig_N = getVoxelDescriptors(slig, buffer=0, voxelsize=self.config['voxelsize'],
                                                          boxsize=self.config['bp_dimension'],
                                                          center=self.config['bp_centers'], validitychecks=False,
                                                          method=self.config['voxel_method'])
